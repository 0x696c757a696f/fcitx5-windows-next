...
mod windows_driver {
    ...
    fn run() -> Result<String, String> {
        ...
        let rect_notes = rect_failures(&case, &root_rect);
        if !rect_notes.is_empty() {
            return fail_case(&mut case.child, &case.com, format!("BoundingRectangle assertions failed for {} element(s); descendants={} buttons={} invoke_capable={}: {}", rect_notes.len(), case.items.len(), case.items.iter().filter(|i| i.control_type == BUTTON).count(), case.items.iter().filter(|i| i.invoke).count(), rect_notes.into_iter().take(5).collect::<Vec<_>>().join(" | ")));
        }
    ...
    unsafe fn refresh_items(com: &mut Com) -> Result<Vec<Item>, String> { if !com.array.is_null() { ((*(*com.array).vtable).release)(com.array); } let hr = ((*(*com.root).vtable).find_all)(com.root, TREE_SCOPE_DESCENDANTS, com.condition, &mut com.array); if hr < 0 || com.array.is_null() { return Err(format!("FindAll HRESULT 0x{hr:08x}")); } let mut len = 0; ((*(*com.array).vtable).length)(com.array, &mut len); let mut out = Vec::with_capacity(len as usize); for i in 0..len { let mut e = null_mut(); let hr = ((*(*com.array).vtable).get_element)(com.array, i, &mut e); if hr < 0 || e.is_null() { return Err(format!("element array index {i} HRESULT 0x{hr:08x}")); } let (rect, rect_error) = rect_soft(e); out.push(Item { element: e, name: property_name(e)?, control_type: property_i4(e, UIA_CONTROL_TYPE_PROPERTY_ID)?, enabled: property_bool(e, UIA_IS_ENABLED_PROPERTY_ID)?, focusable: property_bool(e, UIA_IS_KEYBOARD_FOCUSABLE_PROPERTY_ID)?, rect, rect_error, invoke: has_invoke(e) }); } Ok(out) }
    ...
    fn validate(case: &Case) -> Result<(), String> { let s = &case.strings; let nav_matches: Vec<_> = case.items.iter().filter(|i| i.name == s.nav).collect(); if nav_matches.len() != 1 { return Err(format!("nav.appearance {:?} enumerated {} times, expected exactly one", s.nav, nav_matches.len())); } let nav = nav_matches[0]; if !nav.enabled || !nav.invoke { return Err(format!("nav.appearance {:?} enabled={} invoke_capable={}", s.nav, nav.enabled, nav.invoke)); } let required = [(&s.apply, BUTTON, false, "settings.candidate.apply"), (&s.section, TEXT, false, "settings.candidate.section"), (&s.reset, BUTTON, false, "settings.candidate.reset"), (&s.cancel, BUTTON, false, "settings.candidate.cancel")]; for (name, ty, needs_invoke, key) in required { let matches: Vec<_> = case.items.iter().filter(|i| i.name == *name).collect(); if matches.is_empty() { return Err(format!("missing {key} element named {:?}", name)); } if matches.len() > 1 && (ty == BUTTON || needs_invoke) { return Err(format!("duplicate {key} element named {:?}", name)); } for item in matches { check_item(case, item, ty, needs_invoke, key)?; } }
