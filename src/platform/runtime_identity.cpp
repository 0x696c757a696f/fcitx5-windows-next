...
bool tokenSid(HANDLE process, std::wstring& sid, bool& serviceAccount) {
    ...
    if (required == 0 || required > 64U * 1024U) return false;
...
bool validChannel(std::wstring_view channel) {
    if (channel.empty() || channel.size() > 32) return false;
...
std::wstring normalized(std::wstring_view input) {
    if (input.empty() || input.size() >= 32768) return {};
    std::wstring source(input);
    std::wstring output(32768, L'\0');
    const DWORD length = GetFullPathNameW(source.c_str(), static_cast<DWORD>(output.size()),
                                         output.data(), nullptr);
...
bool RuntimeIdentity::mayUseUserEngine() const noexcept {
    return mayLaunchUserEngine(*this);
}
...
bool queryCurrentIdentity(RuntimeIdentity& output) noexcept {
    ...
    if (!queryProcessIdentity(GetCurrentProcessId(), process)) return false;
    try {
...
std::wstring makeLocalEndpointName(const RuntimeIdentity& identity,
    ...
    if (identity.userSid.empty() || identity.sessionId == 0 || !validChannel(channel)) return {};
    return L"\\\\.\\pipe\\Fcitx5WindowsNext.v2." + identity.userSid + L".Session." +
           std::to_wstring(identity.sessionId) + L"." + std::wstring(channel);
}
