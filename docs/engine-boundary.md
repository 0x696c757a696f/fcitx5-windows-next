...
6. `E5`: move snapshot and surrounding-text canonicalization to Rust. **IN PROGRESS — E5-1 + E5-2 DONE**: the snapshot/status canonicalization pure functions (`content_locale_for_input_method`, `status_short_label`) **and the typed EngineSnapshot DTO limits validation** (`FcitxEngineSnapshotC` flat DTO mirroring `RuntimeResult` + `validate_snapshot` with the protocol payload budgets: commit/preedit 16 KiB, candidates 128, candidate field 4 KiB, locale 35 bytes) are Rust-owned. `FcitxRuntime::Impl` applies them in `collectResult` (content locale, short label, snapshot validation failing closed with `invalid engine snapshot` on budget overflow). Remaining E5: the `pendingStates` per-context snapshot store (put/take with revision ordering) on top of this DTO, and the surrounding-text canonicalization consolidation.
