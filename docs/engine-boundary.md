...
4. `E3`: converge Fcitx event -> plain EngineEvent -> Rust -> EngineActionBatch -> C++ adapter. **IN PROGRESS — E3-1 + E3-2 DONE**: the input-method switch hotkey decision (`classify_input_method_switch`, mirrors the frozen C++ `matches()`) and the candidate-navigation decision (`decide_candidate_action` in `rust/engine-core/src/navigation.rs`, mirrors the frozen C++ candidate block branch-for-branch: scroll digit/`;`/`'` selection, scroll Up/Down + page-key navigation with pageable turns, page-key paging, ordinary `;`/`'` selection, Left/Right highlight moves, Space/Return highlight commit) are Rust-owned with `ImSwitchAction`/`CandidateAction` (SelectAndClear / SetOverride / PageNext|PagePrevAndSetOverride / ConsumeOnly) and the `candidate_navigation.h` target helpers (`row/column/sameColumn*Target`) re-implemented as pure functions. `FcitxRuntime::processKey` now flattens the Fcitx candidate view (`FcitxCandidateViewC`/`FcitxCandidateConfigC`) and executes the returned `FcitxCandidateDecisionC` against Fcitx (`candidate.select`, `pageable->next/prev`, ledger override writes). Corpus frozen in `engine-core-contract` + Rust pure tests (75 total); source-contract requires the two decision ABI calls and blocks the old C++ `matches()` lambda and the C++ navigation helpers from returning. Remaining E3 work: surrounding-text and input-method-selection decisions, then the event-shape consolidation.
