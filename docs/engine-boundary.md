...
4. `E3`: converge Fcitx event -> plain EngineEvent -> Rust -> EngineActionBatch -> C++ adapter. **IN PROGRESS — E3-1 + E3-2 + E3-3 DONE**: the input-method switch hotkey decision (`classify_input_method_switch`), the candidate-navigation decision (`decide_candidate_action` in `rust/engine-core/src/navigation.rs`), the surrounding-text decision (`decide_surrounding_text`), and the input-method-selection decision (`decide_input_method_selection`) are Rust-owned; `FcitxRuntime::processKey` only flattens Fcitx facts and executes the returned actions (`candidate.select`, `pageable->next/prev`, `setCurrentInputMethod`, `surroundingText().setText/invalidate`, ledger writes). Corpus frozen in `engine-core-contract` + Rust pure tests (87 total); source-contract requires the six decision ABI calls and blocks the old C++ decision fragments (`matches()` lambda, navigation helpers, surrounding-text `if` chain, `entry(request.inputMethodUtf8)` selection). Remaining E3 work: the event-shape consolidation (single `EngineEvent`/`EngineActionBatch` container) before E4.
