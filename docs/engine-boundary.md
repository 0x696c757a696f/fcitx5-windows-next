...
5. `E4`: move Engine IPC transport/framing/session/deadline policy to Rust. **IN PROGRESS — E4-1 + E4-2 + E4-3 DONE**: the engine-process session epoch (`EngineEpoch` — `generate_engine_epoch`/`validate_engine_epoch`), the per-connection request ordering (`accept_frame_sequence`, strictly-newer ids), the key-request deadline policy (`key_request_timeout_ms`, 7500 ms cold revision-0 vs 250 ms warm), **and the per-connection session state (`ConnectionSession` in `rust/engine-core/src/session.rs` — `begin_hello` handshake with repeat-handshake/session/process/stale rejection, `accept_frame` handshake-complete/epoch/session/ordering validation, `complete_request` last-accepted-id recording; C ABI `session_create`/`session_destroy`/`session_begin_hello`/`session_accept_frame`/`session_complete_request` with panic containment and null fail-closed)** are Rust-owned in `fcitx5-engine-core`; `fcitx_engine_main.cpp` applies them in `handleRequest` (the C++ `handshakeComplete`/`lastRequestId` locals are deleted and `serve` owns one Rust session per connection). Frozen IPC corpus (this file, `Current Engine Call Graph`): pipe service = 16 worker threads serving one connection each (`CreateNamedPipe`/`verifyPipeClient` via `windows-common-core`); session = `handshakeComplete` + per-connection `lastRequestId` + `clientIdentity.sessionId`/`engineEpoch` frame checks; deadline = per-request dispatcher timeout with fail-open drop at the deadline check. Client-side response-scalar validation already lives in `windows-common-core` (`fcitx5_windows_common_apply_*_response_scalars` in `pipe_client.cpp`). `Generation` (`FCITX5_RELEASE_GENERATION`) stays a release-platform attribute. Remaining E4: transport/framing consolidation (server-side pipe `transfer`/`connectClient`/`readFrame` primitives toward `windows-common-core`).
