...
5. `E4`: move Engine IPC transport/framing/session/deadline policy to Rust. **IN PROGRESS — E4-1 + E4-2 DONE**: the engine-process session epoch (`EngineEpoch` — `generate_engine_epoch`/`validate_engine_epoch`), the per-connection request ordering (`accept_frame_sequence`, strictly-newer ids), and the key-request deadline policy (`key_request_timeout_ms`, 2500 ms cold revision-0 vs 75 ms warm) are Rust-owned in `fcitx5-engine-core`; `fcitx_engine_main.cpp` applies them in `handleRequest`. Frozen IPC corpus (this file, `Current Engine Call Graph`): pipe service = 16 worker threads serving one connection each (`CreateNamedPipe`/`verifyPipeClient` via `windows-common-core`); session = `handshakeComplete` + per-connection `lastRequestId` + `clientIdentity.sessionId`/`engineEpoch` frame checks; deadline = per-request dispatcher timeout with fail-open drop at the deadline check. Client-side response-scalar validation already lives in `windows-common-core` (`fcitx5_windows_common_apply_*_response_scalars` in `pipe_client.cpp`). `Generation` (`FCITX5_RELEASE_GENERATION`) stays a release-platform attribute. Remaining E4: transport/framing consolidation and remaining per-connection session state.
