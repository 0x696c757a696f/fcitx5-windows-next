...
6. `E5`: move snapshot and surrounding-text canonicalization to Rust. **IN PROGRESS — E5-1 DONE**: the snapshot/status canonicalization pure functions are Rust-owned — `content_locale_for_input_method` (`ContentLocale` mapping for mozc/hangul/keyboard-us/rime+pinyin+libime families, mirroring the C++ substring checks in order) and `status_short_label` (two ASCII bytes or the first code point, byte-level tolerating arbitrary input) in `fcitx5-engine-core`, with C ABI `fcitx5_engine_core_content_locale_for_input_method` (enum code) and `fcitx5_engine_core_status_short_label` (buffer write + byte count). `FcitxRuntime::Impl` applies them in `collectResult` (content locale) and `currentInputMethod` (short label); the C++ `contentLocaleForInputMethod`/`statusShortLabel`/`utf8CharacterEnd` helpers are deleted. Remaining E5: the full `RuntimeResult` snapshot DTO canonicalization and the `pendingStates` store (per-context snapshot cache) with the same freeze → side-by-side → cutover discipline.
