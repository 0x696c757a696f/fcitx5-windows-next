...
1. `E0`: freeze current Engine call graph, owner matrix, protocol schema and corpus. **DONE** (`docs/engine-boundary.md`, `docs/tasks/status.md`).
...
4. `E3`: converge Fcitx event -> plain EngineEvent -> Rust -> EngineActionBatch -> C++ adapter. **DONE — CUTOVER-GREEN** (2026-08-24): the four product decisions (input-method switch hotkey `classify_input_method_switch`, candidate navigation `decide_candidate_action` in `rust/engine-core/src/navigation.rs`, surrounding text `decide_surrounding_text`, input-method selection `decide_input_method_selection`) are Rust-owned **and consolidated into a single Event→Action entry `handle_key_event`** (`EngineKeyEvent` flattened facts → `EngineKeyDecision` with surrounding action + input-method selection + main path = im-switch | candidate navigation | forward-with-clear-override, mirroring `processKey` order exactly). `FcitxRuntime::processKey` now calls `fcitx5_engine_core_handle_key_event` once and executes the returned decision (fail-closed forward on ABI error); the individual decision C ABI functions remain exported for corpus compatibility. Corpus frozen in `engine-core-contract` + Rust pure tests (97 total); source-contract requires `handle_key_event` and blocks the old C++ maps and decision fragments. The `candidate_navigation.h` helpers are re-implemented as pure functions in `navigation.rs`.
5. `E4`: move Engine IPC transport/framing/session/deadline policy to Rust. **CUTOVER-GREEN FOR CURRENT E4 SCOPE — E4-1 + E4-2 + E4-3 + E4-4 DONE**: the engine-process session epoch (`EngineEpoch` — `generate_engine_epoch`/`validate_engine_epoch`), the per-connection request ordering (`accept_frame_sequence`, strictly-newer ids), the key-request deadline policy (`key_request_timeout_ms`, 7500 ms cold revision-0 vs 250 ms warm), **and the per-connection session state (`ConnectionSession` in `rust/engine-core/src/session.rs` — `begin_hello` handshake with repeat-handshake/session/process/stale rejection, `accept_frame` handshake-complete/epoch/session/ordering validation, `complete_request` last-accepted-id recording; C ABI `session_create`/`session_destroy`/`session_begin_hello`/`session_accept_frame`/`session_complete_request` with panic containment and null fail-closed)** are Rust-owned in `fcitx5-engine-core`; `fcitx_engine_main.cpp` applies them in `handleRequest` (the C++ `handshakeComplete`/`lastRequestId` locals are deleted and `serve` owns one Rust session per connection). Production server-side overlapped pipe connect and byte transfer now delegate to `windows-common-core` (`fcitx5_windows_common_pipe_connect_client`, `fcitx5_windows_common_pipe_transfer_with_stop`, and `fcitx5_windows_common_deadline_after_milliseconds`); `fcitx_engine_main.cpp` no longer owns duplicate `ConnectNamedPipe`/`ReadFile`/`WriteFile` overlapped loops. Frozen IPC corpus (this file, `Current Engine Call Graph`): pipe service = 16 worker threads serving one connection each (`CreateNamedPipe`/`verifyPipeClient` via `windows-common-core`); session = handshake complete + per-connection last accepted request id + `clientIdentity.sessionId`/`engineEpoch` frame checks; deadline = per-request dispatcher timeout with fail-open drop at the deadline check, plus bounded server connect/header/body/response waits through Rust common pipe primitives. Client-side response-scalar validation already lives in `windows-common-core` (`fcitx5_windows_common_apply_*_response_scalars` in `pipe_client.cpp`). `Generation` (`FCITX5_RELEASE_GENERATION`) stays a release-platform attribute.
6. `E5`: move snapshot and surrounding-text canonicalization to Rust. **DONE — CUTOVER-GREEN** (2026-08-24): the snapshot/status canonicalization pure functions (`content_locale_for_input_method`, `status_short_label`), the typed `EngineSnapshot` DTO limits validation, **and the `pendingStates` per-context snapshot store** (`rust/engine-core/src/snapshot.rs`: canonical blob codec + `SnapshotStore` with put / take-when-request-revision-strictly-older / forget; C ABI `snapshot_store_put`/`take`/`required_size`) are Rust-owned. `FcitxRuntime::Impl` applies them in `collectResult` (locale, short label, snapshot validation fail-closed) and `selectCandidate`/`takePendingState` (snapshot serialized into the Rust store, taken with revision ordering); the C++ `pendingStates` map is deleted.
7. `E6`: delete replaced C++ product state, leaving only the Fcitx adapter and Windows process shell. **DONE — CUTOVER-GREEN** (2026-08-24): the last replaced C++ pure logic is gone — `candidate_navigation.h` and its unit test are deleted (the target helpers and the scroll-mode candidate label offset `scroll_label_offset` are Rust-owned in `navigation.rs`); the C++ `pendingStates` map, ledger maps, Event→Action decision blocks, epoch/sequence/deadline policy, and snapshot/status canonicalization helpers were all deleted in E1–E5. `FcitxRuntime`/`FcitxDispatcher`/`EngineInputContext` remain as the Fcitx adapter (fcitx object access) and `fcitx_engine_main.cpp` as the Windows process shell.
