...
5. `E4`: move Engine IPC transport/framing/session/deadline policy to Rust. **STARTED — E4-1 DONE**: the engine-process session epoch (`EngineEpoch`) is Rust-owned — `generate_engine_epoch` (100ns-since-1601 FILETIME value mirroring `GetSystemTimeAsFileTime`) and `validate_engine_epoch` (frame/process epoch equality) in `fcitx5-engine-core`, C ABI `fcitx5_engine_core_generate_engine_epoch`/`validate_engine_epoch`; `fcitx_engine_main.cpp` calls the generator once at startup and validates every frame's metadata epoch through the validator. `Generation` (`FCITX5_RELEASE_GENERATION`) stays a release platform attribute in `windows-common-core`/platform scope. Remaining E4: IPC transport/framing/session/deadline policy and the response-scalar validation consolidation.
