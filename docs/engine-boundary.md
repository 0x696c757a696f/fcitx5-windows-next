...
4. `E3`: converge Fcitx event -> plain EngineEvent -> Rust -> EngineActionBatch -> C++ adapter. **IN PROGRESS — E3-1 DONE**: the first Event→Action decision moved to Rust. `classify_input_method_switch` (mirroring the frozen C++ `matches()` hotkey logic for `Ctrl+Space`/`Ctrl+Shift`/`Ctrl+Shift+Space`/`Alt+Shift`) is Rust-owned in `fcitx5-engine-core` (`ImSwitchAction::Toggle/Next`, keysym constants from `keysymgen.h`) with a C ABI (`fcitx5_engine_core_classify_input_method_switch`); `FcitxRuntime::processKey` now calls Rust for the hotkey decision and only executes the returned action (toggle/next) against Fcitx. The hotkey decision corpus is frozen in `engine-core-contract` and the Rust pure tests; source-contract requires the ABI call and blocks the old `matches()` lambda from returning. Remaining E3 work: candidate-navigation decision (scroll/page/arrow/space/enter/`;`/`'` → `SelectCandidate`/`SetSelectedOverride`/`ClearSelectedOverride`/page actions) with the same freeze → side-by-side → cutover discipline, then the surrounding-text and input-method-selection decisions.
