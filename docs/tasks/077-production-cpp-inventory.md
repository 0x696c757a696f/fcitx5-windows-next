...
| `src/ipc/pipe_client.cpp`, `pipe_client.h`, `launcher_client.cpp`, `launcher_client.h` | DELETED at 078 Stage 3/4 (engine-E2E slice) | Marshal pipe transport + peer verification against the Rust windows-common + protocol-core C ABI; frame validation, accept/reject, and response scalars were Rust-owned. Local `KeyResult`/`CaretRect`/`EngineStatusResult`/`LauncherResponse` mirrors and per-context `ContextState` were transport request bookkeeping, not product authority. `launcher_client.*` deleted with the launcher-E2E Rust migration; `pipe_client.*` deleted when `fcitx_engine_integration_test` migrated to the Safe Rust `rust/ipc-client/src/bin/engine_e2e.rs` harness (real `fcitx5-engine.exe` + `fcitx5-ui.exe` driven through the frozen protocol). The `fcitx5_ipc_client` STATIC target now contains only `peer_verification.cpp`. |
