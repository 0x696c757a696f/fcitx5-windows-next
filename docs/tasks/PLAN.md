...
| 068 | REPOSITORY-FRESHNESS-MIRROR-IDENTITY-001 | COMPLETED / AUTOMATED-X64-X86-GREEN | 067 package metadata boundary green; extend current v2 repository verification with freshness, freeze/mix-and-match and mirror-identity corpus without claiming TUF | completed/068-REPOSITORY-FRESHNESS-MIRROR-IDENTITY-001.md |
| 069 | PLUGIN-ECOSYSTEM-BUILD-MATRIX-001 | COMPLETED / AUTOMATED-CONTRACTS-GREEN / MANUAL-PENDING | Pinned Lua and Unikey upstream sources now use the standard Fcitx build-farm path and share the existing signed package generator; real production signing/publication and host lifecycle remain external | completed/069-PLUGIN-ECOSYSTEM-BUILD-MATRIX-001.md |
| 070 | LOW-RESOURCE-SLO-CALIBRATION-001 | COMPLETED / CODE-ONLY / EXTERNAL-EVIDENCE-PARTIAL / MANUAL-PENDING | 066 semantics, 068 repository path, and 069 ecosystem matrix green; repeatable Rust low-resource measurement harness and initial SLO evidence are green; real 2-core/4-GB calibration remains manual-pending | completed/070-LOW-RESOURCE-SLO-CALIBRATION-001.md |
| 071 | RUST-TEST-AUTHORITY-CUTOVER-001 | COMPLETED / AUTOMATED-RUST-CORE-X64-X86-GREEN / RETAINED-MIXED-KNOWN-FAILURE / MANUAL-PENDING | Frozen ownership inventory and Rust-authoritative package/update, protocol, launcher, Config, fuzz, corpus, and codec benchmark routes are recorded with separate x64/x86 evidence; retained C++ tests are limited to approved adapters/mixed boundaries. Existing control-package and control-repository-rollback failures are known and not marked green; the x86 updater CLI child also cannot start without elevation in the current environment. | completed/071-RUST-TEST-AUTHORITY-CUTOVER-001.md |
| 072 | CANDIDATE-WIN32-RENDERER-RUST-CUTOVER-001 | COMPLETED / AUTOMATED-RUST-PRESENTATION-X64-X86-GREEN / MANUAL-PENDING / CMAKE-BLOCKED | Rust owns the bounded Candidate presentation-state vertical slice; native Win32/D2D/DWrite/IPC adapters remain, while final host/accessibility/visual/shipping cutover evidence is manual-pending and CMake is blocked by missing pinned WTL headers | completed/072-CANDIDATE-WIN32-RENDERER-RUST-CUTOVER-001.md |
...
| 074 | RELEASE-HOST-EVIDENCE-AND-SIGNING-UAC-001 | IN PROGRESS / PREPARATION-GREEN / MANUAL-PENDING / CI-UNVERIFIABLE | Reuses the completed host, a11y, UAC, plugin, low-resource, and upstream tasks as one frozen-stage evidence campaign: modern host/a11y → signing/UAC → online plugin lifecycle → Win7/low-resource/offline → CI rehearsal. No row is waived without an explicit support-matrix/specification change. | 074-RELEASE-HOST-EVIDENCE-AND-SIGNING-UAC-001.md |
...
| 077 | PRODUCTION-CPP-RUST-CUTOVER-001 | COMPLETED / PROTOCOL-DTO-BRIDGE-DELETED-X64-X86-GREEN / INVENTORY-CLOSED / RUNTIME-SECURITY-RESOLVED / RELEASE-PACKAGE-GREEN / MANUAL-PENDING-HOST-EVIDENCE | 071-076; `REL-01` external evidence remains parked. Full release package gate green from HEAD (x64/x86 Release + real Fcitx acceptance + stage-package → setup.exe + portable.zip). Real-host Accessibility/signing/UAC/Win7 evidence remains MANUAL-PENDING and gates REL-01, not 077. | completed/077-PRODUCTION-CPP-RUST-CUTOVER-001.md |
| 078 | IPC-PLATFORM-RUST-SEAM-CUTOVER-001 | COMPLETED / TRANSPORT-ADAPTERS-DELETED-X64-GREEN / REAL-ENGINE-MANUAL-PENDING | pipe_client/launcher_client deleted (DTO mirrors gone); five wire + both launcher + engine E2E tests migrated to Rust fcitx5-ipc-client; fcitx5_ipc_client now peer_verification-only; remaining C++ is document-allowed durable Fcitx adapter + renderer host + policy-free native seams | completed/078-IPC-PLATFORM-RUST-SEAM-CUTOVER-001.md |
| 079 | UPSTREAM-VENDOR-SUBTREE-001 | COMPLETED / SCRIPTED-VENDOR-ONE-COMMAND-SYNC-GREEN | Independent tooling task; delivered scripted vendoring (not subtree/submodule): `tools/sync-windui.ps1` one-command sync with the two Windows portability patches as named fail-closed files under `third_party/patches/wind-ui-rust/`, `tools/sync-fcitx-pins.ps1` for fcitx5 core/addon pin bumps, and `.github/workflows/sync-upstream.yml` scheduled PR-based auto-sync; fcitx5 core/addons stay pinned build-time checkouts with `$sourcePins` + `third_party/patches/` as the single source of truth | 079-UPSTREAM-VENDOR-SUBTREE-001.md |
...
| 081A | RENDERER-TINY-SKIA-FN-001 | COMPLETED / RENDER-FN-AND-ABI-GREEN | 080; Rust render_candidate_window fn + C ABI + unit tests (preedit/label/text/comment/selection/scrollbar/border from axis_layout rects + candidate data + theme) | 081-RENDERER-D2D-TINY-SKIA-CUTOVER-001.md |
...
| 081C | RENDERER-HWND-RUST-001 | COMPLETED / RUST-HWND-WNDPROC-PUMP-STATE-DECISIONS-MEASURE-VISUAL-X64-X86-GREEN / MANUAL-PENDING-HOST-VISUAL | 080; Rust owns class registration, HWND create/destroy, WndProc structural dispatch, message pump, pointer/scroll/click-guard/focus-watch state, presentation decide chain, measure engine + measure loop, visual build arena (incl. scroll-label reservations), paint colors, combined render+blit, window assembly, and natural-width downgrade; C++ shell is the allowed native seam; real-host visual evidence remains MANUAL-PENDING | 081-RENDERER-D2D-TINY-SKIA-CUTOVER-001.md |
| 081D | RENDERER-DELETE-CPP-001 | COMPLETED / UI-MAIN-DELETED-HOST-SHELL-X64-X86-GREEN / MANUAL-PENDING-HOST-VISUAL | 081C; `src/ui/ui_main.cpp` deleted (renamed `src/ui/ui_host.cpp`, the document-allowed native seam: Win32 glue + FFI adapters + pipe server + mixed-binary harness); candidate-core 81/81; six self-tests + parent-close smoke exit 0 in both lanes; real-host visual parity MANUAL-PENDING (REL-01) | 081-RENDERER-D2D-TINY-SKIA-CUTOVER-001.md |
| 082 | CANDIDATE-UI-HOST-RUST-BINARY-001 | COMPLETED / RUST-SHIPPING-BINARY-UI-HOST-CPP-DELETED-X64-79-79-X86-78-79-GREEN / MANUAL-PENDING-HOST-VISUAL | 7bd81e3 | Slice 1: windows-common-core gains full peer verification (`verified_pipe_client` — SID/path/final-path/file identity, two-phase like the C++ contract), `NamedPipeServer::verified_client`, and public `paths_refer_to_same_file`; candidate-core gains `presentation_server` (frozen 64-byte/256 KiB wire constants, decode + header-validated frame reads, endpoint naming, engine-executable gate, serve loop with stop responsiveness and test-once semantics). Unit coverage: encode/decode round trip + truncated/wrong-type/oversized/body-mismatch rejection. candidate-core 84/84 + windows-common-core 62/62; x64/x86 fcitx5_ui builds; six self-tests exit 0 both lanes. | Slice 2: Rust frame orchestration ABI (model→presentation→visual→measure→layout→assembly). |
| 083 | ENGINE-SNAPSHOT-CODEC-RUST | COMPLETED / FLAT-ABI-NO-WIRE-FORMAT-ACROSS-FFI-X64-79-79-X86-78-79-GREEN | 53c3cb4 |
| 084 | UNSAFE-SAFETY-COMMENT-COVERAGE | COMPLETED / UNSAFE-SAFETY-COVERAGE-GREEN | 083; strict Clippy reports zero undocumented unsafe blocks; workspace Rust tests, rustfmt, and x64 CTest 79/79 pass without product-logic changes | completed/084-UNSAFE-SAFETY-COMMENT-COVERAGE.md |
| 085 | CANDIDATE-WECHAT-THEME-CONSISTENCY-001 | COMPLETED / CANDIDATE-PILL-AND-GLYPH-GREEN | 060-062 and 080-084; fixes the alpha-lost/green-on-green shipping contrast defect and aligns builtin theme, Qingfeng, renderer fallback, Settings preview, and PoC imagery without changing High Contrast or release evidence status | completed/085-CANDIDATE-WECHAT-THEME-CONSISTENCY-001.md |
| 086 | CANDIDATE-LAYOUT-MODE-SETTINGS-001 | IN PROGRESS | 085; turns the already-Rust-owned `automatic`, `stacked`, `flow`, `scroll`, and `vertical_text` model into concise, persistent Settings choices with a production-path live preview. No new layout engine or arbitrary grid. | 086-CANDIDATE-LAYOUT-MODE-SETTINGS-001.md |
| REL-01 | RELEASE-01 | RELEASE-GATED / EXTERNAL-EVIDENCE-PENDING | 050 + all 074 evidence-campaign rows + intended Rust cutovers; it signs/promotes only the frozen Build Once stage and never recompiles | release/REL-01-RELEASE-GATE.md |
...
- `074` is the single active external-evidence campaign. Its ordered batches prevent duplicate
  task creation: freeze one stage; modern host/a11y; signing/UAC; online plugin lifecycle; Win7,
  low-resource, storage, and network resilience; then GitHub Actions rehearsal. `REL-01` begins
  only after every required row is recorded against that stage. A requirement may be removed only
  by an explicit support-matrix/specification edit, never because local automation is green.
