...
| 082 | CANDIDATE-UI-HOST-RUST-BINARY-001 | OPEN / USER-CHALLENGE-RECORDED | 081; user challenge: ui_host.cpp is still 2.8k lines of C++ (update orchestration, pipe server, config adapter, harness). Make fcitx5-ui.exe a Rust shipping binary (precedent: fcitx5-config.exe) and delete ui_host.cpp. Slices: pipe server → frame orchestration ABI → config consumption → window host integration → Rust main. | 082-CANDIDATE-UI-HOST-RUST-BINARY-001.md |
