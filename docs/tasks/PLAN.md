...
| 077 | PRODUCTION-CPP-RUST-CUTOVER-001 | IN-PROGRESS / PROTOCOL-DTO-BRIDGE-DELETED-X64-X86-GREEN / INVENTORY-CLOSED / RUNTIME-SECURITY-TEST-LOOPBACK-RESOLVED / RELEASE-PACKAGE-REBASELINE-PIN-BLOCKED / FULL-CUTOVER-REQUIRED | 071-076; `REL-01` external evidence remains parked and does not block code migration. Runtime-security source audit green (test-only loopback exempted with a comment). Release package gate builds x64/x86 Release from final HEAD but cannot run the real-Fcitx acceptance until `out/sources/fcitx5` is at the official `cdd0b9d` pin (rollback checkout with user changes is preserved). Do not archive until the release-package item is unblocked. | 077-PRODUCTION-CPP-RUST-CUTOVER-001.md |
| 078 | IPC-PLATFORM-RUST-SEAM-CUTOVER-001 | TODO / NOT-SELECTED | 077 source-cutover; delete or reduce the remaining C++ IPC/platform marshalling mirrors (`pipe_client.*`, `launcher_client.*`, `runtime_identity.*`, `peer_verification.*`, `pipe_security.*`) to direct Rust ABI consumers with no local DTO mirror; the direct Fcitx adapter island and Candidate renderer host remain C++ | 078-IPC-PLATFORM-RUST-SEAM-CUTOVER-001.md |
