...
| 079 | UPSTREAM-VENDOR-SUBTREE-001 | TODO / NOT-SELECTED | Independent tooling task; switch `third_party/wind-ui-rust` to a git subtree/submodule tracked at one pin with the two Windows portability patches stored as named files and re-applied fail-closed, and provide one-command sync that updates pin/deps.json/Cargo.lock/reference-commit and runs affected tests; keep fcitx5 core/addons as pinned build-time checkouts with `$sourcePins` + `third_party/patches/` as the single source of truth | 079-UPSTREAM-VENDOR-SUBTREE-001.md |
