...
## Completion evidence (078 DONE)

Transport adapters deleted: pipe_client.cpp/.h (KeyResult/CaretRect/EngineStatusResult DTO mirrors) and
launcher_client.cpp/.h (LauncherResponse mirror) are gone. Five pure-wire ipc_* tests, both launcher
E2E tests, and the fcitx_engine_integration_test release-gate acceptance are migrated to Rust
(rust/ipc-client: safe EngineClient + launcher command client + engine_e2e harness, all six scenarios
baseline/safe-mode/first-run-rime/rime-lua/typing-fuzz/chttrans preserved verbatim). fcitx5_ipc_client STATIC
now holds only peer_verification.cpp (native seam). Remaining C++ is document-allowed: durable Fcitx
adapter island (fcitx_engine_main.cpp), renderer host (ui_main.cpp), and the policy-free native seams
(runtime_identity/peer_verification/pipe_security, Rust-authoritative FFI marshal).

Verified: cargo test -p fcitx5-ipc-client 11/11; cargo build --bin engine_e2e; ninja full +
fcitx5_engine_e2e green; cargo fmt -p fcitx5-ipc-client -p fcitx5-windows-common-core; git diff --check.
Real-engine acceptance (fcitx5-engine.exe via bootstrap-fcitx.ps1) remains release-gated/manual, not faked.
