...
|---|---|---|
| PR | Every pull request and push to `main`, GitHub-hosted Windows 2022, PowerShell 7 | Affected x64/x86 clean configure/build, warning-as-error, focused CTest contracts/integration/fuzz smoke, PE/import hardening, secrets, dependency, license, locale and UTF-8/LF policies |
| Nightly | Scheduled clean checkout without signing secrets | Broader unit/integration/fuzz/perf smoke, real Fcitx/Rime/Lua/addon acceptance, immutable x64+x86 package stage, installer/portable construction and portable relocation test |
| Win7 VM milestone | Explicit compatibility milestone on pinned Legacy VM/toolchain | Legacy lane import/min-OS check plus x86/x64 TSF registration, Notepad input, basic candidate/config, repair/uninstall and declared Win7 limitations |
| Stable | Protected, tag-bound environment with signing authority | Promote the already tested stage only; targeted Windows 10/11 desktop smoke, Authenticode, hashes, detached manifest, SBOM, provenance, rollback and distribution metadata all verify |

The CI matrix is risk-selected, not Cartesian. Do not multiply every OS, architecture, host,
DPI and input method together by default; add a cell only when the changed boundary can actually
fail there or a regression has made that cell evidence-bearing.
