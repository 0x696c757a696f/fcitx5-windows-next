...
| Engine E2 `engine-core` ledger | `CUTOVER-GREEN` (Rust authoritative; `FcitxRuntime::Impl` cut over, C++ maps deleted) | Extend to remaining per-context maps (`carets`/`popupAllowed`/`pendingStates`/`selectedOverride`/`inputMethodOverridden`) and `EngineEpoch`/`Generation`; then E3 Event→Action |
...
- Engine product state is migrating to Rust: E1 protocol codec is cut over (`CUTOVER-GREEN`); E2 ledger is cut over (`CUTOVER-GREEN`, `FcitxRuntime::Impl` uses the Rust ledger and the C++ maps are deleted); the remaining per-context maps, `EngineEpoch`/`Generation`, and the Fcitx adapter shrink remain.
...
3. Engine E1 + E2 are cut over at this HEAD: `protocol/protocol.cpp` is a thin C ABI bridge over the Rust `protocol-core` codec (wire frozen in `protocol_wire_golden.inc`), and `FcitxRuntime::Impl` uses the Rust `engine-core` ledger (`engine_core_ffi.h`) with the C++ `nextCompositionId`/`compositions`/`revisions` maps deleted; `engine-core-contract` pins the frozen ledger semantics and the real `fcitx5-engine.exe` integration acceptance (baseline/typing-fuzz/safe-mode/chttrans) passes on the native-engine lane. Next step: extend the E2 ledger to the remaining per-context maps and `EngineEpoch`/`Generation`, then E3 (Fcitx event -> plain EngineEvent -> Rust -> EngineActionBatch -> C++ adapter).
