...
| Engine E3 Event→Action | `PARTIAL` (E3-1 + E3-2 `CUTOVER-GREEN`: input-method switch + candidate-navigation decisions Rust-owned; surrounding-text/input-method-selection decisions still C++) | Move surrounding-text and input-method-selection decisions to Rust with the same freeze→side-by-side→cutover discipline |
