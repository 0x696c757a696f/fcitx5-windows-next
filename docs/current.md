...
| Engine E3 Event→Action | `PARTIAL` (E3-1..E3-3 `CUTOVER-GREEN`: input-method switch + candidate-navigation + surrounding-text + input-method-selection decisions Rust-owned) | Consolidate the event shape (single EngineEvent/EngineActionBatch container) before E4 |
