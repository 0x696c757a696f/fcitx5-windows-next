...
| Engine E3 Event→Action | `PARTIAL` (E3-1 `CUTOVER-GREEN`: input-method switch hotkey decision Rust-owned; candidate navigation/surrounding-text decisions still C++) | Move candidate-navigation, surrounding-text, and input-method-selection decisions to Rust with the same freeze→side-by-side→cutover discipline |
