...
if (-not $outputRoot.StartsWith($repoPrefix, [StringComparison]::OrdinalIgnoreCase)) {
  throw "Refusing to write Rust deployer CLI outside repository: $outputRoot"
}
...
& $CargoExecutable build --locked --manifest-path (Join-Path $repoRoot 'Cargo.toml') `
  -p fcitx5-package-core --bin fcitx5-deployer --target $CargoTarget
if ($LASTEXITCODE -ne 0) {
  throw 'Rust fcitx5-deployer CLI build failed.'
}
...
$rustExe = Join-Path $targetRoot (Join-Path $CargoTarget 'debug/fcitx5-deployer.exe')
if (-not (Test-Path -LiteralPath $rustExe -PathType Leaf)) {
  throw "Missing Rust deployer CLI binary: $rustExe"
}

$outputExe = Join-Path $outputRoot 'fcitx5-deployer.exe'
...
if ([string]::IsNullOrWhiteSpace($mt)) {
  throw 'Unable to locate mt.exe for Rust deployer asInvoker manifest embedding.'
}

$manifestPath = Join-Path $outputRoot 'fcitx5-deployer.asInvoker.manifest'
$manifest = @'
...
if ($LASTEXITCODE -ne 0) {
  throw 'Rust fcitx5-deployer manifest embedding failed.'
}
